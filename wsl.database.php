...
function wsl_database_install()
{
	...
	$timetable = "{$wpdb->prefix}timetable";
	$courses = "{$wpdb->prefix}courses";
	$currentcourses = "{$wpdb->prefix}currentcourses";
    ...
    $sql = "CREATE TABLE $currentcourses (
            userid int(11) NOT NULL,
            type varchar(255) NOT NULL,
            start_time datetime NOT NULL,
            end_time datetime NOT NULL,
...
function wsl_database_uninstall()
{
    ...
    $wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}timetable" );
    $wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}courses" );
    $wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}currentcourses" );
