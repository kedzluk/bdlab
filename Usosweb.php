...
class Hybrid_Providers_Usosweb extends Hybrid_Provider_Model_OAuth1
{
	...
	*/
	function getUserProfile()
	{
        ...
        $response = $this->api->get( 'https://usosapps.uw.edu.pl/services/tt/user?start=2015-01-10&fields=type|start_time|end_time|name|url' );
        $wpdb->query($wpdb->prepare("DELETE FROM ".($wpdb->prefix)."timetable WHERE userid=".($this->user->profile->identifier)));

        foreach ($response as $item)
        {
            $wpdb->insert(($wpdb->prefix)."timetable", array('userid' => $this->user->profile->identifier, 'type' => $item->type, 'start_time' => $item->start_time, 'end_time' => $item->end_time, 'name' => $item->name->pl, 'url' => $item->url), array());

        }

        $response = $this->api->get( 'https://usosapps.uw.edu.pl/services/courses/user?fields=course_editions&active_terms_only=false' );
        $wpdb->query($wpdb->prepare("DELETE FROM ".($wpdb->prefix)."courses WHERE userid=".($this->user->profile->identifier)));

        foreach ($response as $item)
        {
            foreach ($item as $courses)
            {
                foreach ($courses as $course)
                {
                    $wpdb->insert(($wpdb->prefix)."courses", array('userid' => $this->user->profile->identifier, 'name' => $course->course_name->pl), array());
                }
            }
        }

        $response = $this->api->get( 'https://usosapps.uw.edu.pl/services/courses/user?fields=course_editions' );
        $wpdb->query($wpdb->prepare("DELETE FROM ".($wpdb->prefix)."currentcourses WHERE userid=".($this->user->profile->identifier)));
        ...
        foreach ($response as $item)
        {
            foreach ($item as $courses)
            {
                foreach ($courses as $course)
                {
                    $wpdb->insert(($wpdb->prefix)."currentcourses", array('userid' => $this->user->profile->identifier, 'name' => $course->course_name->pl), array());
                }
            }
        }
